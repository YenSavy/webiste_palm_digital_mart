...
const VerifyOTPForm: React.FC = () => {
  ...
  // Create a helper function to format phone number (same as combinePhoneNumber)
  const formatPhoneNumber = (prefix: string, phoneNum: string): string => {
    const fullPhoneNumber = (prefix + phoneNum).replace(/\D/g, '');
    const phoneWithoutCountryCode = fullPhoneNumber.replace(/^855/, '');
    return phoneWithoutCountryCode;
  };
  
  const fullPhone = formatPhoneNumber(prefix, phone);
  ...
  const validateOTP = (): boolean => {
    ...
    if (otpString.length !== 4) {
      setErrors({ otp: t("otp length") || "Please enter all 4 digits" });
      return false;
    }
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        setIsSuccess(true);
        setIsFailed(false);
        setMessage(t("otp verified") || "Verification successful!");
        ...
        setIsFailed(true);
        setIsSuccess(false);
        setMessage(response.message || t("invalid otp code") || "Invalid verification code.");
      }
    } catch (error: any) {
      setIsFailed(true);
      setIsSuccess(false);
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setMessage(t("otp resent") || "New verification code has been sent.");
      } else {
        setIsFailed(true);
        setMessage(response.message || t("failed to resend otp") || "Failed to resend verification code.");
      }
    } catch (error: any) {
      setIsFailed(true);
      setMessage(error.message || t("network error") || "Network error. Please try again.");
              ...
              {isSuccess && (
                <div className="p-4 bg-green-900/30 border border-green-700 rounded-lg">
                  ...
                  <p className="text-green-200 text-sm mt-1">
                    {t("redirecting to reset") || "Redirecting to reset password page..."}
                  </p>
                </div>
              )}
